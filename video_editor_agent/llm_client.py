...
Everything else in video_editor_agent calls generate() from here - never a
...
This mirrors script_agent/llm_client.py, video_production_agent/llm_client.py,
and voice_generation_agent/llm_client.py, but is its own independent copy -
none of the four agents share code or state.

To add a new text-generation provider later:
  1. Write video_editor_agent/llm_providers/openai_provider.py implementing
...
def _build_provider():
    ...
    if name == "anthropic":
        from video_editor_agent.llm_providers.anthropic_provider import AnthropicProvider

        return AnthropicProvider()

    if name == "manual":
        from video_editor_agent.llm_providers.manual_provider import ManualProvider
