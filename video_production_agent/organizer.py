"""Saves a production manifest back into its source Script Agent folder.
...
Layout (added to an existing scripts/<topic-slug>_<date>/ folder):
  scripts/
    <topic-slug>_<date>/
      ...
      production_manifest.json   <- written by this agent
...
def save_manifest(manifest, folder):
    """Write production_manifest.json into `folder`. Returns the file path."""
    folder = Path(folder)
