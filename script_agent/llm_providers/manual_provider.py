...
the terminal. Nothing else in script_agent knows the difference - swap to
the Anthropic API later by changing LLM_PROVIDER in .env. No code changes.

Tip: paste each prompt into the SAME Claude Pro conversation, in order.
Claude will remember earlier scenes on its own, so continuity stays intact
without us needing to re-send previous scene text.
"""
...
PROMPTS_DIR = Path(__file__).resolve().parent.parent.parent / "scripts" / "_manual_prompts"
